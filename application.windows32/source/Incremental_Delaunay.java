...
public class Incremental_Delaunay extends PApplet {
...
class halfEdge {
  ...
  public void displayFace() {
    ...
    line(face.x, face.y, face.x + (destination.x - origin.x)*.3f ,face.y + (destination.y - origin.y)*.3f );
    
  }
}

public boolean onFace( vertex p, halfEdge hE){
...
public float twoDeterminant ( float a, float b, float c, float d){
  return (a*d) - (b*c);
}

public boolean checkLD( halfEdge hE){
  if(hE.twin == null) {
  return true;
  }
  else {
...
public void flippier(vertex p, halfEdge hE){
...
public void drawCircumCircle(vertex A, vertex B, vertex C){
...
public void makeFirstTriangle(vertex p) {
    vertex vertexA;
    vertex vertexB;
    vertex vertexC;
    halfEdge A;
...
public triangleNode triangulate(vertex p){
...
public void splittier(triangleNode node, vertex p) {
...
public void split(triangleNode node, vertex p){
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Incremental_Delaunay" };
    if (passedArgs != null) {
